...
export type ACTIONTYPE =
  ...
  | { type: typeof SET_LIST_ITEMS; payload: { listItems: DealList[] } };
...
const findList = (id: string | null, listItems: DealList[]) => {
  ...
  const listId = listItems
    .flatMap((list) => list.deals.map((deal: any) => ({ dealId: deal.id, listId: list.id })))
...
export const dealsReducer = (state: DealsState, action: ACTIONTYPE): DealsState => {
  switch (action.type) {
    ...
    case DRAG_OVER: {
      ...
      const activeIndex = activeDeals.findIndex((deal: any) => deal.id === activeId);
      const overIndex = overDeals.findIndex((deal: any) => deal.id === overId);
      ...
      return {
        ...
        listItems: state.listItems.map((item) => {
          if (item.id === activeList.id) {
            return { ...item, deals: activeDeals.filter((dealItem: any) => dealItem.id !== activeId) };
    ...
    case DRAG_END: {
      ...
      if (state.draggedDeal && activeList.id === overList.id) {
        const activeIndex = activeList.deals.findIndex((deal: any) => deal.id === activeId);
        const overIndex = activeList.deals.findIndex((deal: any) => deal.id === overId);
    ...
    case SET_LIST_ITEMS: {
      // Preserve compactMode state when updating list items from API
      const existingCompactModes = new Map(
        state.listItems.map((item) => [item.id, item.compactMode]),
      );
      return {
        ...state,
        listItems: action.payload.listItems.map((item) => ({
          ...item,
          compactMode: existingCompactModes.get(item.id) ?? item.compactMode,
        })),
      };
    }
