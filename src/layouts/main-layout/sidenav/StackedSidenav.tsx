...
const StackedSidenav = () => {
  ...
  const isMenuActive = useCallback((item: MenuItem) => {
    const checkLink = (subMenuItem: SubMenuItem): boolean => {
      if (
      ...
      return subMenuItem.items ? subMenuItem.items.some(checkLink) : false;
    };
      ...
      <Stack sx={{ height: 1 }}>
        ...
        >
              ...
              {sitemap.map((item) => (
                ...
                >
                  <IconButton
                    ...
                    color="primary"
                    aria-label={item.subheader || `Navigate to ${item.id}`}
                    sx={[
                      {
          ...
          <Toolbar sx={{ padding: '0 !important', display: 'flex', justifyContent: 'center' }}>
            <IconButton sx={{ color: 'text.secondary' }} onClick={toggleNavbarCollapse} aria-label={sidenavCollapsed ? 'Expand sidebar' : 'Collapse sidebar'}>
              <IconifyIcon
                icon={
                  sidenavCollapsed
