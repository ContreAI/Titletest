...
const AudioPlayer = ({ src }: { src: string }) => {
  ...
  const handleSeek = (_: Event | React.SyntheticEvent, newValue: number | number[]) => {
  ...
  const handleSeekCommit = (_: Event | React.SyntheticEvent, newValue: number | number[]) => {
  ...
  return (
    ...
    >
      ...
      <IconButton onClick={togglePlayPause} color="primary" aria-label={isPlaying ? 'Pause audio' : 'Play audio'}>
        <IconifyIcon
          icon={
            isPlaying
      ...
      />
      <Stack gap={0.5}>
        <IconButton onClick={toggleMute} aria-label={isMuted ? 'Unmute audio' : 'Mute audio'}>
          <IconifyIcon
            icon={
              isMuted
        ...
        <IconButton onClick={(e) => setAnchorEl(e.currentTarget)} aria-label="More audio options">
          <IconifyIcon
