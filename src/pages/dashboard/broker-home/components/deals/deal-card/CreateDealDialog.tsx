...
const CreateDealDialog = () => {
  ...
  useEffect(() => {
    reset(initialData);
  }, [createDealDialog, initialData, reset]);

  const onSubmit = (data: any) => {
    console.log(data);
    // Convert form data to Deal format
    const deal: Deal = {
      ...data,
      title: data.name, // Map name to title for display
    };
    dealsDispatch({ type: ADD_NEW_DEAL, payload: { listName: data.stage, deal } });
              ...
              render={({ field: { value, onChange }, fieldState }) => (
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    error={!!fieldState.error}
                    onChange={(event) => {
                      const company = companies.find((item: any) => item.name === event.target.value);
                      onChange(company);
                    }}
                  >
